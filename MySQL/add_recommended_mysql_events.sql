...
-- ----------------------------
-- Event structure for removeDeadPlayers
-- ----------------------------
DROP EVENT IF EXISTS `removeDeadPlayers`;
DELIMITER ;;
CREATE EVENT `removeDeadPlayers` ON SCHEDULE EVERY 1 DAY COMMENT 'Removes dead players' DO DELETE FROM `Character_DATA` WHERE Alive = 0
;;
DELIMITER ;
...
DELIMITER ;;
CREATE EVENT `setDamageOnAge` ON SCHEDULE EVERY 1 DAY COMMENT 'This sets damage on a wall so that it can be maintained' DO UPDATE `Object_DATA` SET `Damage`=0.1 WHERE `ObjectUID` <> 0 AND `CharacterID` <> 0 AND `Datestamp` < DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 9 DAY) AND ( (`Inventory` IS NULL) OR (`Inventory` = '[]') OR (`Classname` IN ('Land_DZE_GarageWoodDoorLocked','Land_DZE_LargeWoodDoorLocked','Land_DZE_WoodDoorLocked','CinderWallDoorLocked_DZ','CinderWallDoorSmallLocked_DZ','Plastic_Pole_EP1_DZ')) )
;;
DELIMITER ;
